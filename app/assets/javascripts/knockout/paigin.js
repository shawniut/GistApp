function PagingVM(options) {
    ...
    self.ResultInfo = ko.pureComputed(function () {
        if(self.TotalCount() == 0)
            return 'Showing '+ 0 +' to '+ 0 +' of '+ 0 +' ';
        if(self.TotalCount() <= self.PageSize())
            return 'Showing '+ self.PageSize()*(self.CurrentPage()-1) +' to '+ self.TotalCount() +' of '+ self.TotalCount() +' ';
        return 'Showing '+ self.PageSize()*(self.CurrentPage()-1) +' to '+ self.PageSize()*(self.CurrentPage()) +' of '+ self.TotalCount() +' ';
    });
